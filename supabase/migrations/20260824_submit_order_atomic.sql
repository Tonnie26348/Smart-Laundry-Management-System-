-- Migration: 20260824_submit_order_atomic.sql

-- Drop obsolete function if it exists
DROP FUNCTION IF EXISTS public.submit_order(UUID, JSONB, TEXT, TEXT, TEXT, TEXT, DECIMAL);

-- Atomic RPC function
CREATE OR REPLACE FUNCTION public.submit_order_atomic(
  p_customer_id UUID,
  p_items JSONB, -- Expected: [{"laundry_item_id": UUID, "service_id": UUID, "quantity": INTEGER}]
  p_pickup_address_data JSONB, -- Expected: {"address_line1": TEXT, "address_line2": TEXT, "city": TEXT, "postal_code": TEXT}
  p_delivery_address_data JSONB -- Expected: {"address_line1": TEXT, "address_line2": TEXT, "city": TEXT, "postal_code": TEXT}
)
RETURNS JSONB AS $$
DECLARE
  v_customer_id UUID;
...
BEGIN
  ...
  LOOP
    ...
    IF v_mapping_id IS NULL THEN
        RAISE EXCEPTION 'No active pricing mapping found for item % and service %', v_item.laundry_item_id, v_item.service_id;
    END IF;
  ...
  -- 7. Create Deliveries
  INSERT INTO public.deliveries (order_id, customer_id, pickup_address_id, delivery_address_id, delivery_type, status)
  VALUES (v_order_id, v_customer_id, v_pickup_addr_id, v_delivery_addr_id, 'pickup', 'pending');

  -- 8. Create Payment
  INSERT INTO public.payments (order_id, amount, status, phone_number)
  VALUES (v_order_id, v_total_price, 'pending', (SELECT phone FROM public.customers WHERE id = v_customer_id));
